...
export class SubmissionsPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad SubmissionsPage');
  }
  ...
  listSubmissionSafetyData(){
    this._DB.find({
      selector: {DoYouWishToReport: {$eq: 'Safety Incident'}, Sent: {$eq: 'sent'}},
      sort: ['_id']
    }).then(filtered => {
  ...
  listSubmissionHazardData(){
    this._DB.find({
      selector: {DoYouWishToReport: {$eq: 'Hazard'}, Sent: {$eq: 'sent'}},
      sort: ['_id']
    }).then(filtered => {
  ...
  listSubmissionWellBeingData(){
    this._DB.find({
      selector: {DoYouWishToReport: {$eq: 'Wellbeing'}, Sent: {$eq: 'sent'}},
      sort: ['_id']
    }).then(filtered => {
  ...
  listSubmissionUnsafeBehaviourData(){
    this._DB.find({
      selector: {DoYouWishToReport: {$eq: 'Unsafe Behaviour'}, Sent: {$eq: 'sent'}},
      sort: ['_id']
    }).then(filtered => {
  ...
  listSubmissionSecurityIncidentData(){
    this._DB.find({
      selector: {DoYouWishToReport: {$eq: 'Security Incident'}, Sent: {$eq: 'sent'}},
      sort: ['_id']
    }).then(filtered => {
  ...
  listSubmissionPositiveBehaviourData(){
    this._DB.find({
      selector: {DoYouWishToReport: {$eq: 'Positive Behaviour'}, Sent: {$eq: 'sent'}},
      sort: ['_id']
    }).then(filtered => {
  ...
  listSubmissionSecurityGoodPracticeData(){
    this._DB.find({
      selector: {DoYouWishToReport: {$eq: 'Security Good Practice'}, Sent: {$eq: 'sent'}},
      sort: ['_id']
    }).then(filtered => {
  ...
  listSubmissionSecurityNonComplianceData(){
    this._DB.find({
      selector: {DoYouWishToReport: {$eq: 'Security Non Compliance'}, Sent: {$eq: 'sent'}},
      sort: ['_id']
    }).then(filtered => {
  ...
  DraftReorder($event){
   ...
   presentPopover(myEvent){
    let popover = this.popOverCtrl.create('PopOverSentPage');
    popover.present({
      ev: myEvent
    });
  }
